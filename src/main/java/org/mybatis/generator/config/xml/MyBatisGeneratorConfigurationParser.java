    ...
    private void parseJavaModelGenerator(Context context, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                ...
                Properties attributeList = parseAttributes(childNode);
                if("javaExampleTargetPackage".equals(attributeList.getProperty("name"))){
                    javaExampleGeneratorConfiguration.setTargetPackage(attributeList.getProperty("value"));
                }else if("javaExampleTargetProject".equals(attributeList.getProperty("name"))){
                    javaExampleGeneratorConfiguration.setTargetProject(attributeList.getProperty("value"));
                }
            }
        }
    }
